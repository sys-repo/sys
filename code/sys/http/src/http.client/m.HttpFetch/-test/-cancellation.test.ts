import { describe, expect, it, Json, Schedule, Testing, type t } from '../../../-test.ts';
...
describe('Http.Fetch: cancellation authority', () => {
  ...
  it('abort during checksum preparation → canonical path-safe 499', async () => {
    ...
    globalThis.fetch = () => Promise.resolve(new Response(new Uint8Array([0, 1, 2])));
  ...
  it('removes composed-signal listeners after successful settlement', async () => {
    ...
    globalThis.fetch = () => Promise.resolve(new Response('done'));
