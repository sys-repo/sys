...
export async function loadConfig(path: t.StringPath): Promise<t.HttpStatic.ConfigDoc> {
  const read = await Fs.readText(path);
  if (!read.ok) {
    throw new Error(`${ERROR_PREFIX}: failed to read config: ${Fs.trimCwd(path)}`);
  }
...
function parseConfigText(text: string, path: t.StringPath): t.HttpStatic.ConfigDoc {
  ...
  if (parsed.error) {
    throw new Error(`${ERROR_PREFIX}: invalid config YAML: ${Fs.trimCwd(path)}`, {
      cause: parsed.error,
    });
  }
  ...
  if (!Is.record(data)) {
    throw new Error(`${ERROR_PREFIX}: config must be a YAML object: ${Fs.trimCwd(path)}`);
  }
...
function assertKnownKeys(doc: Record<string, unknown>, path: t.StringPath): void {
  ...
  if (unknown.length > 0) {
    throw new Error(`${ERROR_PREFIX}: unknown config key '${unknown[0]}': ${Fs.trimCwd(path)}`);
  }
}
...
function readNonEmptyString(
  ...
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(
      `${ERROR_PREFIX}: config key '${key}' must be a non-empty string: ${Fs.trimCwd(path)}`,
    );
  }
  return value.trim();
}
...
function readPort(value: unknown, path: t.StringPath): number {
  if (!Is.num(value) || !Number.isInteger(value) || value < 0 || value > 65535) {
    throw new Error(
      `${ERROR_PREFIX}: config key 'port' must be an integer between 0 and 65535: ${
        Fs.trimCwd(path)
      }`,
    );
  }
