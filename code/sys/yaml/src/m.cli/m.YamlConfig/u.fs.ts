import { type t, Fs, Yaml } from './common.ts';
import { type YamlConfigSchema } from './t.menu.ts';

export const DEFAULT_EXT = '.yaml';
...
export function fileLabel(path: t.StringPath, ext = DEFAULT_EXT): string {
  return stripExt(Fs.basename(path), ext);
}

export function fileOf(name: string, ext = DEFAULT_EXT): string {
  let base = name.trim();
