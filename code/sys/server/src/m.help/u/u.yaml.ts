...
export const HelpYaml = {
  record(text: string, path: string): O {
    ...
    if (parsed.error) {
      ...
      throw new Error(`ServerHelp: failed to parse resource YAML: ${path}`, { cause });
    }

    const data = parsed.data;
    if (!Is.record<O>(data)) {
      throw new Error(`ServerHelp: resource must be a YAML record: ${path}`);
    }
    return data;
  },
  ...
  string(data: O, field: string): string {
    ...
    if (!Is.str(value)) throw new Error(`ServerHelp: field must be a string: ${field}`);
  ...
  list(data: O, field: string): readonly string[] {
    ...
    if (!Is.array<string>(value) || !value.every(Is.str)) {
      throw new Error(`ServerHelp: field must be a string list: ${field}`);
    }
    return value;
  },
  ...
  pairs(data: O, field: string): readonly Pair[] {
    ...
    if (!Is.array<O>(value) || !value.every(isPairRecord)) {
      throw new Error(`ServerHelp: field must be a pair record list: ${field}`);
    }
  ...
  require(data: O, fields: readonly string[]) {
    fields.forEach((field) => {
      if (!(field in data)) throw new Error(`ServerHelp: missing field: ${field}`);
    });
  },
} as const;
