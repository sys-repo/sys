...
describe('DistServer.start', () => {
  describe('startup and failure', () => {
    ...
    it('refuses missing and mismatched generations with typed sanitized failures', async () => {
      ...
      try {
        ...
        expect(DistServer.Error.is(missing)).to.eql(true);
        expect(missing?.name).to.eql('DistServer.StartError');
