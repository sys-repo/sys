...
`;
    ...
    it('empty', () => {
      const test = (input: P) => {
        const ignore = GlobIgnore.create(input);
    ...
    it('from multi-line string (eg. a file)', () => {
      const a = GlobIgnore.create(gitignore);
      const b = GlobIgnore.create([gitignore]);
      const c = GlobIgnore.create(['  ', gitignore, '', '\n\n']);
    ...
    it('multi-line within array', () => {
      const ignoreA = GlobIgnore.create(gitignore);
      const ignoreB = GlobIgnore.create([gitignore, '  node_modules  ', '', ' \n\n']);
  ...
  describe('pattern rules', () => {
    ...
    it('removes comments', () => {
      const gitignore = `
        ...
        .env.*
      `;
      const ignore = GlobIgnore.create(gitignore);
  ...
  describe('isIgnored → (".gitignore" complaint)', () => {
    const test = (pattern: string | string[], path: string, expected: boolean) => {
      const gitignore = GlobIgnore.create(pattern);
