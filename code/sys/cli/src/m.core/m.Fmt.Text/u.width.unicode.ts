 ...
 * Unicode updates must change the provenance explicitly and repeat scalar and composite parity.
 */
const fullwidthRanges = TextIntrinsic.freeze([
  12288,
  12288,
  65281,
  65376,
...
const wideRanges = TextIntrinsic.freeze([
  4352,
  4447,
  8986,
  8987,
...
/** Resolve one scalar to the default narrow-ambiguous terminal width. */
export function eastAsianCellWidth(codePoint: number): 1 | 2 {
  return isInRanges(codePoint, fullwidthRanges, 12288, 65510) ||
      isInRanges(codePoint, wideRanges, 4352, 262141)
    ? 2
    : 1;
}
TextIntrinsic.freeze(eastAsianCellWidth);
...
function isInRanges(
  ...
  if (codePoint < minimum || codePoint > maximum) return false;
