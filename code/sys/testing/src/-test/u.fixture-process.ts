import { Arr, Err, Is, Num, Obj, Str, stripAnsi, Time } from './common.ts';
import type { t } from './common.ts';

export type FixtureProcessOptions = {
...
export type FixtureProcessTimeout = {
...
export type FixtureProcessResult = {
  ...
  readonly timeout?: FixtureProcessTimeout;
...
/** Run one Deno fixture with independently bounded startup, execution, and output-drain phases. */
export async function runFixtureProcess(
...
async function completeChild(
  ...
  return toResult(status, drain.value, marker, elapsed);
}
...
async function terminateChild(
  ...
  elapsed: t.Msecs,
  drainTimeout: t.Msecs,
  phase: FixtureProcessTimeout['phase'],
  label: string,
): Promise<FixtureProcessResult> {
  ...
  return toResult(capturedStatus, capturedOutput, marker, elapsed, { phase, killError });
}
...
async function captureOutput(
  ...
  const [stdout, stderr] = await Promise.allSettled(
    [
      readOutput(child.stdout, onStdout),
