...
describe('VClock - Virtual Clock', () => {
  ...
  describe('VClock.makeForTotal', () => {
    it('total-only clock clamps and never maps to source', () => {
      const clock = VClock.makeForTotal(1_000 as t.Msecs, { startAt: 2_000, loop: true, speed: 1 });

      // startAt clamps to total-1 (exclusive end)
      expect(VTime.toMsecs(clock.get().vtime)).to.eql(999);

      // mapping is intentionally unavailable
      expect(clock.mapToSource(VTime.fromMsecs(0))).to.eql(null);
      expect(clock.mapToSource(VTime.fromMsecs(500))).to.eql(null);

      // loop works (999 + 10) % 1000 = 9
      const s1 = clock.advance(10 as t.Msecs);
      expect(VTime.toMsecs(s1.vtime)).to.eql(9);
    });
  });
});
