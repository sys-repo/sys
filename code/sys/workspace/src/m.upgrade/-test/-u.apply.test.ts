...
describe('Workspace.Upgrade.apply', () => {
  ...
  it('reuses registry fetches across upgrade planning and apply in one session', async () => {
    ...
    const counted = {
      jsr: {
        versions: async (...args: Parameters<typeof registry.jsr.versions>) => {
          versionsCalls += 1;
          return await registry.jsr.versions(...args);
        },
        info: async (...args: Parameters<typeof registry.jsr.info>) => {
          infoCalls += 1;
          return await registry.jsr.info(...args);
        },
      },
      npm: {
        versions: async (...args: Parameters<typeof registry.npm.versions>) => {
          versionsCalls += 1;
          return await registry.npm.versions(...args);
        },
        info: async (...args: Parameters<typeof registry.npm.info>) => {
          infoCalls += 1;
          return await registry.npm.info(...args);
        },
      },
    };
    const session = fixture.session(counted);
