 ...
 * - `--json` is unstable, so this schema intentionally validates only the
 *   fields the workspace graph relies on.
 *
...
export function validateInfoJson(value: unknown): t.WorkspaceGraphCli.InfoJson {
  if (Schema.Value.Check(DenoInfoJsonSchema, value)) return value;
