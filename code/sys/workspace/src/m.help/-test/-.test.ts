...
describe('WorkspaceHelp', () => {
  ...
  it('loads the test-runner ownership and handoff contracts exactly', async () => {
    ...
    expect(scheduler?.items).to.eql([
      ...
      'Live completed rows use newest-first scheduler terminal-event order. Each new row enters at top-left; prior visible rows shift row-major toward bottom-right; the oldest moves into `...and N more`.',
