...
export async function sync(
  args: t.WorkspaceCi.Test.Windows.SyncArgs,
): Promise<t.WorkspaceCi.SyncResult> {
  const cwd = args.cwd ?? Fs.cwd();
  const paths = await resolveSourcePaths(cwd, args.source, { task: 'test:windows' });
  if (paths.length === 0) {
    ...
    logSyncResult('test:windows', result, { log: args.log });
    return result;
  }

  const res = await write({ cwd, on: args.on, paths, target: args.target });
