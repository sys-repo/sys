...
describe('KeyValue.Cursor', () => {
  ...
  describe('scope resolution', () => {
    it('resolves root cursor scope with groups as atoms', () => {
      ...
      expect(scope.items.map((item) => item.parts)).to.eql([['key', 'value'], [], [
        'key',
        'value',
      ]]);
  ...
  describe('movement: lanes', () => {
    ...
    it('collapses row lanes while moving vertically through items', () => {
      const rowItems: Item[] = [
        { id: 'alpha', k: 'alpha' },
        { id: 'bravo', k: 'bravo' },
      ...
      expect(Cursor.previous({ current: partTarget('key', 'alpha') }, rowItems).current).to.eql({
        path: ['alpha'],
        part: 'key',
      });
    ...
    it('drops row lanes on vertical movement without treating them as nested scopes', () => {
      const scoped: Item[] = [
        { id: 'alpha', k: 'alpha' },
        { id: 'bravo', k: 'bravo' },
        { id: 'divider', kind: 'hr' },
        {
          ...
          kind: 'group',
          items: [
            { id: 'child.a', k: 'child.a' },
            { id: 'child.group', kind: 'group', items: [{ id: 'grandchild', k: 'grandchild' }] },
            { id: 'child.b', k: 'child.b' },
          ],
        },
        { id: 'charlie', k: 'charlie' },
      ];

      expect(Cursor.previous({ current: partTarget('key', 'bravo') }, scoped).current).to.eql({
        path: ['alpha'],
      });
      expect(Cursor.next({ current: partTarget('value', 'alpha') }, scoped).current).to.eql({
        path: ['bravo'],
      });
      expect(Cursor.next({ current: partTarget('value', 'bravo') }, scoped).current).to.eql({
        path: ['divider'],
      });
      expect(Cursor.next({ current: target('group') }, scoped).current).to.eql({
        path: ['charlie'],
      });
      expect(Cursor.next({ current: partTarget('value', 'group', 'child.a') }, scoped).current).to
        .eql({ path: ['group', 'child.group'] });
      expect(Cursor.next({ current: partTarget('key', 'group', 'child.b') }, scoped).current).to
        .eql({
          path: ['group', 'child.b'],
          part: 'key',
        });
  ...
  describe('movement: blocks', () => {
    it('moves by hr-delimited block edges while collapsing row lanes', () => {
      ...
      expect(Cursor.nextBlock({ current: partTarget('value', 'bravo') }, blockItems).current).to
        .eql({
          path: ['charlie'],
        });
      ...
      expect(Cursor.previousBlock({ current: partTarget('value', 'charlie') }, blockItems).current)
        .to.eql({ path: ['bravo'] });
      expect(Cursor.previousBlock({ current: partTarget('key', 'bravo') }, blockItems).current).to
        .eql({ path: ['alpha'] });
      expect(Cursor.previousBlock({ current: partTarget('key', 'alpha') }, blockItems).current).to
        .eql({ path: ['alpha'], part: 'key' });
      expect(Cursor.previousBlock({ current: target('alpha') }, blockItems).current).to.eql({
        path: ['alpha'],
      });
      expect(Cursor.nextBlock({ current: partTarget('value', 'delta') }, blockItems).current).to
        .eql({
          path: ['delta'],
          part: 'value',
        });
  ...
  describe('movement: edges', () => {
    it('moves to current-scope top and bottom while collapsing row lanes', () => {
      expect(Cursor.first({ current: partTarget('key', 'alpha') }, items).current).to.eql({
        path: ['alpha'],
        part: 'key',
      });
      ...
      expect(Cursor.last({ current: partTarget('value', 'charlie') }, items).current).to.eql({
        path: ['charlie'],
        part: 'value',
      });
  ...
  describe('commands', () => {
    it('routes data-only cursor commands through cmd', () => {
      ...
      const right: t.KeyValue.Cursor.Command<'cursor:right'> = {
        name: 'cursor:right',
        payload: {},
      };
