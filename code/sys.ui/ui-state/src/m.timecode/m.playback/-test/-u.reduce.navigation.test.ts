...
describe('Playback.reduce — navigation', () => {
  function initState() {
    return reduce(emptyState(), { kind: 'playback:init', timeline: timeline() }).state;
  }
  ...
  it('seek:beat clamps below 0 to 0', () => {
    ...
    const state1 = reduce(initState(), { kind: 'playback:seek:beat', beat: 1 }).state;

    const res = reduce(state1, { kind: 'playback:seek:beat', beat: -100 });
  ...
  it('seek:beat clamps above max to max', () => {
    ...
    const res = reduce(state, { kind: 'playback:seek:beat', beat: 999 });
  ...
  it('next advances by +1 (clamped)', () => {
    const state0 = reduce(initState(), { kind: 'playback:seek:beat', beat: 0 }).state;

    const res1 = reduce(state0, { kind: 'playback:next' });
    expect(res1.state.currentBeat).to.eql(1);

    const evt1 = beatEvent(res1.events);
    expect(evt1?.beat).to.eql(1);

    const res2 = reduce(res1.state, { kind: 'playback:next' });
    expect(res2.state.currentBeat).to.eql(2);

    const evt2 = beatEvent(res2.events);
    expect(evt2?.beat).to.eql(2);

    const res3 = reduce(res2.state, { kind: 'playback:next' });
  ...
  it('prev moves by -1 (clamped)', () => {
    const state2 = reduce(initState(), { kind: 'playback:seek:beat', beat: 2 }).state;

    const res1 = reduce(state2, { kind: 'playback:prev' });
    expect(res1.state.currentBeat).to.eql(1);

    const evt1 = beatEvent(res1.events);
    expect(evt1?.beat).to.eql(1);

    const res2 = reduce(res1.state, { kind: 'playback:prev' });
    expect(res2.state.currentBeat).to.eql(0);

    const evt2 = beatEvent(res2.events);
    expect(evt2?.beat).to.eql(0);

    const res3 = reduce(res2.state, { kind: 'playback:prev' });
  ...
  it('navigation never changes phase', () => {
    ...
    const a = reduce(state, { kind: 'playback:seek:beat', beat: 1 });
    expect(a.state.phase).to.eql(phase);

    const b = reduce(a.state, { kind: 'playback:next' });
    expect(b.state.phase).to.eql(phase);

    const c = reduce(b.state, { kind: 'playback:prev' });
