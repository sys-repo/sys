 ...
 */
export async function cloneTemplate(cwd: t.StringDir, variant: t.PullTool.TemplateVariant) {
  ...
  const name = await Cli.Input.Text.prompt('Pull → ToolName');
  const id = await Cli.Input.Text.prompt({
    message: 'pull → tool-id',
