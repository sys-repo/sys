...
export const ROOT_REGISTRY = [
  ...
  it('applies yaml variant overlay to generated files', async () => {
    ...
    try {
      ...
      await Fs.write(Fs.join(dir, 't.namespace.ts'), nsTemplate.replaceAll('Pull', name));
      await Fs.write(Fs.join(dir, 'm.cli.ts'), cliTemplate.replaceAll('Pull', name));
      await Fs.write(Fs.join(dir, 'common.ts'), commonTemplate.replaceAll('Pull', name));
  ...
  it('generated variants pass deno check', async () => {
    ...
    const run = async (variant: t.PullTool.TemplateVariant) => {
      const tmp = await Fs.makeTempDir();
