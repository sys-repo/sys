...
/**
 * Migration 04:
 * - rename top-level `remoteBundles` to `bundles`.
 */
export async function migrate04(
  cwd: t.StringDir,
  ...
  for (const entry of files) {
    ...
    const hasOld = Array.isArray(root.remoteBundles);
    const hasNew = Array.isArray(root.bundles);
    if (!hasOld || hasNew) {
      skipped.push({ from: path, to: path });
      continue;
    }

    root.bundles = root.remoteBundles;
