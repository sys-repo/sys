  ...
  it('resolves only configured mutable output directories', async () => {
    await withTmpDir(async (cwd) => {
      ...
      expect(resolved.localDirs.map((dir) => dir.dir)).to.eql([
        'view/.pulled/sample.package',
        './view/releases/fixture',
      ]);
      expect(resolved.localDirs.map((dir) => dir.path)).to.eql([
        Fs.join(cwd, 'workspace/view/.pulled/sample.package'),
...
async function withTmpDir<T>(fn: (cwd: t.StringDir) => Promise<T>): Promise<T> {
  const dir = await Fs.makeTempDir({ prefix: 'sys.tools.pull.materialize.' });
  try {
