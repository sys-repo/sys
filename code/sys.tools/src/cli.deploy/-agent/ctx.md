...
Staging builds a deployable filesystem tree based on mappings. Push resolves targets from staged
outputs, then deploys each target via provider.
...
- Config authority is syntax-gated: `${env:NAME}` resolves dotenv/process env, `~`/`~/` source paths
  resolve HOME, and ordinary paths request neither.
