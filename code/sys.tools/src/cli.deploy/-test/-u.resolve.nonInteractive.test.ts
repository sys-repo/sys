...
describe('@sys/tools/deploy non-interactive resolution', () => {
  it('resolves --config and maps stage+push to the shared run action', async () => {
    await withTmpDir(async (cwd) => {
      const yamlRel = '-config/@sys.tools.deploy/sample.yaml';
      await Fs.ensureDir(`${cwd}/src/site`);
      await Fs.write(
        `${cwd}/src/site/index.html`,
        '<!doctype html><html><body>sample</body></html>\n',
      );
      await Fs.write(
        `${cwd}/${yamlRel}`,
        Str.dedent(
...
/** Helpers: */
async function withoutProcessEnv(key: string, fn: () => Promise<void>): Promise<void> {
  const original = Deno.env.get(key);
  Deno.env.delete(key);
  try {
    await fn();
  } finally {
    if (original !== undefined) Deno.env.set(key, original);
  }
}
