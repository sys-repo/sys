 ...
 */
export async function cloneTemplate(cwd: t.StringDir, variant: t.TmplTool.TemplateVariant) {
  ...
  const name = await Cli.Input.Text.prompt('Tmpl → ToolName');
  const id = await Cli.Input.Text.prompt({
    message: 'tmpl → tool-id',
