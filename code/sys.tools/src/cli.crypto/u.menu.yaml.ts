...
import { CryptoFs, CryptoYamlSchema } from './u.yaml/mod.ts';
...
export async function yamlConfigsMenu(cwd: t.StringDir): Promise<YamlConfigsMenuPick> {
  const schema = {
    init: () => CryptoYamlSchema.initial(),
    validate: (value: unknown) => CryptoYamlSchema.validate(value),
  } as const;

  const res = await YamlConfig.menu<t.CryptoTool.ConfigYaml.Doc, Action>({
    cwd,
    dir: CryptoFs.dir,
    label: 'Crypto Config',
    itemLabel: 'config',
    ...
    add: {
      ...
      initYaml: ({ name: configName }) => CryptoFs.initialYaml(configName),
    },
  });
...
function nameFromPath(path: t.StringPath): string {
  ...
  return base.endsWith(CryptoFs.ext) ? base.slice(0, -CryptoFs.ext.length) : base;
}
