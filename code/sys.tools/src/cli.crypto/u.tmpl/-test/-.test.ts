...
export const ROOT_REGISTRY = [
  ...
  it('applies yaml variant overlay to generated files', async () => {
    ...
    try {
      ...
      await Fs.write(Fs.join(dir, 't.namespace.ts'), nsTemplate.replaceAll('Crypto', name));
      await Fs.write(Fs.join(dir, 'm.cli.ts'), cliTemplate.replaceAll('Crypto', name));
      await Fs.write(Fs.join(dir, 'common.ts'), commonTemplate.replaceAll('Crypto', name));
  ...
  it('generated variants pass deno check', async () => {
    ...
    const run = async (variant: t.CryptoTool.TemplateVariant) => {
      const tmp = await Fs.makeTempDir();
