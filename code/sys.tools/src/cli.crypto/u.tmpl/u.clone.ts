 ...
 */
export async function cloneTemplate(cwd: t.StringDir, variant: t.CryptoTool.TemplateVariant) {
  ...
  const name = await Cli.Input.Text.prompt('Crypto → ToolName');
  const id = await Cli.Input.Text.prompt({
    message: 'crypto → tool-id',
