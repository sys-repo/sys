import { type t, c, Cli, Fmt, pkg, Str } from './common.ts';
import { isToolCommand } from './u.is.ts';
import { rootRows } from './u.rows.ts';

export type RootMenuPick =
  | { readonly kind: 'exit' }
  | { readonly kind: 'selected'; command: t.Tools.Command };
...
export async function rootMenu(): Promise<RootMenuPick> {
  ...
  const exitIndent = ' '.repeat(Fmt.Tree.branch([0, rows]).length);

  rows.forEach((row, index) => {
    const branch = index === total - 1 ? Fmt.Tree.branch(true, 1) : Fmt.Tree.branch([index, rows]);
  ...
  const options: Array<{ name: string; value: t.Tools.Command | 'exit' }> = rows.map(
    (row, index) => ({
  ...
  console.info();
  const picked = await Cli.Input.Select.prompt<t.Tools.Command | 'exit'>({
    message: [
