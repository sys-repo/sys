...
## Status
Active narrow design note.
This note is the closest `-agent` plan for the new internal seam under `src/u.passThrough/`.

## Purpose
Extract the shared local-vs-published delegation decision out of thin `@sys/tools/*` pass-through wrappers without prematurely abstracting wrapper-specific command assembly.

Initial sample callers only:
...
This type layer is intentionally small and structural.
It is the proof root for the extraction, not the final consumer refactor.
...
However, aliases do **not** belong in the pass-through delegation context.
They belong in the root registry / dispatcher layer.
...
Examples:
- `@sys/tools code` → delegates to `@sys/driver-agent/pi/cli` with injected `Profiles` and help env
...
- `cli.code` adopts it without changing wrapper-specific command policy
- alias/index concerns remain clearly documented as root-layer ownership, not leaked into pass-through context
