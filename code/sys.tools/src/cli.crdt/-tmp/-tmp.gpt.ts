...
const Fmt = {
  ...BaseFmt,
  headerTable(model?: string, subjectId?: t.Crdt.Id, apiKey?: string) {
    const key = apiKey ? Str.ellipsize(apiKey, [3, 4], '..') : '-';
    const table = Cli.table();
    const kv = (k: string, value: t.Json = '') => table.push([c.gray(k), String(value)]);
    kv('model', c.white(model || '-'));
    kv('api.key', c.gray(key));
    kv('document', c.gray(subjectId ? Fmt.prettyUri(subjectId) : '-'));
...
export async function tmp(cwd: t.StringDir, docid: t.Crdt.Id, yamlPath: t.ObjectPath) {
  ...
  async function run(life: t.Lifecycle) {
    ...
    async function render(): Promise<void> {
      ...
      const seq = await Slug.MediaComposition.Sequence.fromDag(dag, yamlPath, subjectId);
      const scripts = (seq ?? [])
      ...
      const { text } = await generateText({
        ...
        system: [
          ...
          'Zen like responses with profound, clear, considered and short responses.',
