...
/**
 * Load the root hook module from the given directory, if present.
 */
export async function loadRootHook(
  cwd: t.StringDir,
  filename = D.Hook.filename,
): Promise<t.RootHookModule | undefined> {
  ...
  return mod as t.RootHookModule;
}
