...
describe('OptimizeImportsPlugin.rewriteImports', () => {
  ...
  it('rewrites an approved named import to a public subpath', () => {
    const input = "import { useKeyboard } from '@sys/ui-react-devharness/react';";
  ...
  it('groups multiple approved imports targeting the same subpath', () => {
    const input = "import { useKeyboard, useRubberband } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);

    expect(result.changed).to.eql(true);
    expect(result.code).to.eql(
      "import { useKeyboard, useRubberband } from '@sys/ui-react-devharness/react/hooks';",
    );
  });

  it('preserves unknown imports on the broad package root', () => {
    const input = "import { Dev, useKeyboard } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);

    expect(result.changed).to.eql(true);
    expect(result.code).to.eql(Str.dedent(`
      import { useKeyboard } from '@sys/ui-react-devharness/react/hooks';
      import { Dev } from '@sys/ui-react-devharness/react';
    `));
  });

  it('leaves unknown imports unchanged', () => {
    const input = "import { Dev } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);
  ...
  it('preserves aliases when rewriting', () => {
    const input = "import { useKeyboard as useDevKeyboard } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);

    expect(result.changed).to.eql(true);
    expect(result.code).to.eql(
      "import { useKeyboard as useDevKeyboard } from '@sys/ui-react-devharness/react/hooks';",
    );
  });
  ...
  it('leaves namespace imports unchanged', () => {
    const input = "import * as DevHarness from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);
  ...
  it('keeps type-only imports on the root when only value rewrites are approved', () => {
    const input = "import type { useKeyboard } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);
  ...
  it('rewrites mixed value and type specifiers with per-kind behavior', () => {
    const input = "import { type HookSpec, useKeyboard, Dev } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);

    expect(result.changed).to.eql(true);
    expect(result.code).to.eql(Str.dedent(`
      import { useKeyboard } from '@sys/ui-react-devharness/react/hooks';
      import type { HookSpec } from '@sys/ui-react-devharness/react/hooks';
      import { Dev } from '@sys/ui-react-devharness/react';
    `));
  });

  it('rewrites whole-declaration import type when a type rule exists', () => {
    const input = "import type { HookSpec } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);

    expect(result.changed).to.eql(true);
    expect(result.code).to.eql("import type { HookSpec } from '@sys/ui-react-devharness/react/hooks';");
  });

  it('rewrites whole-declaration import type when a both-kind rule exists', () => {
    const input = "import type { KeyboardEventSpec } from '@sys/ui-react-devharness/react';";
    const result = rewriteImports(input, rules);

    expect(result.changed).to.eql(true);
    expect(result.code).to.eql(
      "import type { KeyboardEventSpec } from '@sys/ui-react-devharness/react/hooks';",
    );
  });
  ...
  it('rewrites formatted multi-line declarations', () => {
    const input = Str.dedent(`
      import {
        useKeyboard,
        Dev,
      } from '@sys/ui-react-devharness/react';
    `);
    const result = rewriteImports(input, rules);
