...
describe('DevScreen', () => {
  describe('header identity', () => {
    it('keeps version visible by dropping package scope before dropping version', () => {
    ...
    it('renders the visible package identity as a green structural header lane', () => {
      const pkgName = '@sys/ui-components';
      const version = '0.0.319';
      ...
      expect(rawHeader).to.not.include(c.white(c.bold(pkgName)));
      expect(rawHeader).to.not.include(c.gray(version));
      expect(rawHeader).to.not.include(c.bold(c.cyan(version)));
    });
  });

  describe('ready frame layout', () => {
    it('renders a compact stable frame with bounded dev log rows', () => {
      ...
      expect(header.startsWith('@sys/example')).to.eql(true);
      expect(header.endsWith('0.0.0')).to.eql(true);
      expect(header).to.not.include('Dev');
  ...
  describe('startup handoff', () => {
    it('renders a startup snapshot with a spinner line and truthful parent status row', () => {
      ...
      expect(rows[0]).to.not.include('Dev');
    ...
    it('drives startup through an injectable spinner and stops it on dispose', () => {
      ...
      expect(printed[0].startsWith('@sys/example')).to.eql(true);
      expect(printed[0]).to.not.include('Dev');
      expect(spinner.starts).to.eql(1);
      expect(spinner.text.startsWith('\n\n')).to.eql(false);
      expect(stripAnsi(spinner.text)).to.include(` 1  out  ${STARTING_DEV_SERVER}`);
