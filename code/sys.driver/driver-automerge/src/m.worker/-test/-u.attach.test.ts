...
describe('CrdtWorker.attach', { sanitizeResources: false, sanitizeOps: false }, () => {
  ...
  it('emits a health snapshot on attach', async () => {
    const { port1, port2 } = Test.makePorts();
    const repo = Test.realRepo();
    const { events, stop } = Test.collectRepoEvents(port1);

    CrdtWorker.attach(port2, repo);
    await Wait.waitFor(() => events.some((e) => e.type === 'health'));
    ...
    expect(healthEvents.length).to.be.greaterThan(0);

    const first = healthEvents[0];
    expect(first.payload.busy).to.eql(false);
    expect(typeof first.payload.lastProgressAt).to.eql('number');

    stop();
    await repo.dispose();
  });

  it('emits busy=true then busy=false around a worker RPC call', async () => {
    const { port1, port2 } = Test.makePorts();
    const real = Test.realRepo({ network: true });
    const { events, stop } = Test.collectRepoEvents(port1);
    const client = CrdtWorker.repo(port1);
    ...
    type HealthEvent = Extract<t.WireRepoEventPayload, { type: 'health' }>;
    const healthEvents = events.filter((e) => e.type === 'health') as HealthEvent[];
    const last = healthEvents.at(-1)!;

    expect(last.payload.busy).to.eql(false);
