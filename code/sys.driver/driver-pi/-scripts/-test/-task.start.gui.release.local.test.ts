...
describe('driver-pi/scripts/task.start.gui.release.local', () => {
  ...
  it('pins least-authority evidence binding', () => {
    ...
    expect(permissions['evidence-local']).to.eql({
      read: ['./dist'],
      write: ['./src/m.cli/m.profiles/u.start/u.gui/u.service.evidence.ts'],
    });
  ...
  it('confines disposable runtime writes, wildcard bind, and fixed-port authority', () => {
    ...
    expect(permissions['test-release-launch']).to.eql({
      env: ['CHROME_BIN'],
      run: ['deno'],
    });
    expectTaskExact('test:release:local:runtime', [
      'deno run',
      '--frozen',
      '--cached-only',
      '--no-prompt',
      '-P=test-release-launch',
      './-scripts/task.test.release.runtime.ts',
    ]);
  });
  ...
  it('keeps current browser proof build-owning and freezes finite Chrome authority', () => {
    ...
    expectTaskExact(
      ...
      [
        ...
        '--no-prompt',
        '-P=test-release-launch',
        './-scripts/task.test.release.browser.ts',
      ],
    );
  });
});
