...
describe(`@sys/driver-pi/cli/Profiles/m.main`, () => {
  ...
  it('main → starts from the profile menu using the already-rendered sandbox preview', async () => {
    ...
    try {
      ...
      const reportFiles = (await Fs.ls(Fs.join(cwd, '.log', '@sys.driver-pi') as t.StringDir))
  ...
  it('main → migrates legacy profile directory before direct --profile resolution', async () => {
    const prev = Process.inherit;
    const prevInfo = console.info;
    const cwd = (await Fs.makeTempDir({ prefix: 'driver-pi.profiles.m.main.test.' }))
      .absolute as t.StringDir;
    ...
    try {
      ...
      expect(res.kind).to.eql('run');
      const printed = Cli.stripAnsi(calls.join('\n'));
      expect(printed).to.contain('Migrated 1 Pi profile config.');
      expect(printed).to.contain('pi:sandbox');
    } finally {
      Process.inherit = prev;
      console.info = prevInfo;
