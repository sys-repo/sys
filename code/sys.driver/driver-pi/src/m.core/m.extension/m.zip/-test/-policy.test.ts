...
import { expectRefusal, withRoot } from './u.fixture.ts';

describe('Pi: ZIP policy', () => {
  describe('resolved authority', () => {
    ...
    it('absent roots → retain lexical scope without creating roots or canonical evidence', async () => {
      await withRoot(async (root) => {
        const absent = Fs.join(root, 'not-created');
    ...
    it('prompt → appends only when enabled and reports fixed runtime limits', async () => {
      ...
      expect(text).to.include(
        `${policy.zipLimits.maxExpandedBytes / 1_048_576} MiB total expansion`,
      );
  ...
  describe('untrusted input admission', () => {
