...
import { DenoImports } from './mod.ts';

describe('DenoImports', () => {
  const SAMPLE = {
  ...
  describe('fromYaml', () => {
    it('input: path (string)', async () => {
      const res = await DenoImports.fromYaml(SAMPLE.path);
    ...
    it('input: YAML (string)', async () => {
      ...
      const a = await DenoImports.fromYaml(yaml);
      const b = await DenoImports.fromYaml(path);
      expect(a).to.eql(b);
    ...
    describe('errors', () => {
      it('path: not found', async () => {
        const path = './404.yaml';
        const res = await DenoImports.fromYaml(path);
      ...
      it('invalid YAML: parse error', async () => {
        const yaml = `
          imports:
            ...
            import: npm:rxjs@7
      `;
        const res = await DenoImports.fromYaml(yaml);
      ...
      it('invalid YAML: {imports} not found', async () => {
        const res = await DenoImports.fromYaml('foo:123');
