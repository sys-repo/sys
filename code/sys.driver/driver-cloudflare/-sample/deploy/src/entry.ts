...
/**
 * Resolve credentials for captured inputs,
 * then bootstrap without opening a listener.
 */
export function appFrom(inputs: t.AppInputs, env: t.EnvReader = Deno.env, signal?: AbortSignal) {
  const captured = snapshotInputs(inputs.config, inputs.buildRecord);
...
/**
 * Load package-local inputs once; hosting uses process
 * env unless a reader is supplied.
 */
