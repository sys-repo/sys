...
describe('FilesMemory.readonly', () => {
  ...
  it('keeps default policy deny-all while leaving capabilities observable', async () => {
    const backing = FilesMemory.readonly({ files: { 'foo.txt': 'hello' } });
  ...
  it('rejects unsafe source and command paths with memory-scoped errors', async () => {
    await expectFilesMemoryError(
      () => FilesMemory.readonly(null as never),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly(new Map() as never),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly({ files: 'nope' as never }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly({ files: new Map() as never }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly({ dirs: 'nope' as never }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly({ files: { '../escape.txt': 'nope' } }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly({ files: { 'safe/../escape.txt': 'nope' } }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly({ files: { 'bad\\path.txt': 'nope' } }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => FilesMemory.readonly({ files: { 'docs': 'file', 'docs/readme.md': 'child' } }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () =>
    ...
    const { backing } = setup();
    await expectFilesMemoryError(
      () => cmd.stat(backing, { path: '/etc/passwd' as t.Files.StringPath }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
      () => cmd.stat(backing, { path: 'notes/..' as t.Files.StringPath }),
      'FilesMemoryError.InvalidPath',
    );
    await expectFilesMemoryError(
