...
describe('FilesMemory.writable: write/remove', () => {
  it('advertises writable write/remove capabilities without live watch', async () => {
    const backing = FilesMemory.writable({ policy: allowAllMutablePolicy });
  ...
  it('writes complete text-file values and keeps list/stat/read as truth', async () => {
    const backing = FilesMemory.writable({ dirs: ['docs'], policy: allowAllMutablePolicy });
  ...
  it('writes complete byte-file values without routing bytes through JSON', async () => {
    const backing = FilesMemory.writable({ dirs: ['images'], policy: allowAllMutablePolicy });
  ...
  it('removes files and recursive directories through bounded remove authority', async () => {
    const backing = FilesMemory.writable({ dirs: ['docs/tmp'], policy: allowAllMutablePolicy });
  ...
  it('enforces write/remove policy before mutating memory state', async () => {
    const backing = FilesMemory.writable({
    ...
    await expectFilesMemoryError(
      () => cmd.write(backing, { kind: 'text', path: 'docs/private/readme.md', content: 'nope\n' }),
      'FilesMemoryError.PolicyDenied',
    );
    await expectFilesMemoryError(
