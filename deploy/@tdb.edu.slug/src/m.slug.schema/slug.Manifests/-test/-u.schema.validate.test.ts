...
describe('SlugManifests validation', () => {
  describe('validateAssetsManifest', () => {
    it('accepts a minimal assets manifest', () => {
      const result = validateAssetsManifest(validAssets);
    ...
    it('rejects manifests with an invalid asset kind', () => {
      const result = validateAssetsManifest(invalidAssets);
  ...
  describe('validatePlaybackManifest', () => {
    it('accepts the minimal playback manifest shape', () => {
      const result = validatePlaybackManifest(validPlayback);
      expect(result.ok).to.eql(true);
      if (!result.ok) throw result.error;
    ...
    it('rejects manifests missing required fields', () => {
      const result = validatePlaybackManifest(invalidPlayback);
      expect(result.ok).to.eql(false);
      if (!result.ok) {
        expect(result.error).to.be.instanceOf(Error);
        expect(result.error.message.length > 0).to.eql(true);
