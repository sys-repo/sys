...
describe('LintProfileMigrate.-01', () => {
  it('moves pkg-scoped YAMLs into flattened dir', async () => {
    const tmp = await Fs.makeTempDir();
    try {
      const legacyDir = `${tmp.absolute}/-config/${pkg.name}/lint`;
      await Fs.ensureDir(legacyDir);
      await Fs.write(`${legacyDir}/alpha.yaml`, 'facets: []\n');
      ...
      const nextDir = `${tmp.absolute}/-config/${root}/lint`;
