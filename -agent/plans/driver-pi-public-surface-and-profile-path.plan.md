...
Closed. Public export polish and profile/runtime path migration are landed and verified.

This follows the README / JSR summary cleanup and should be executed as small mechanical commits. No
compatibility aliases: this is greenfield cleanup.
...
- Historical starting truth was documented before export cleanup:
  ...
  - profiles resolved to `-config/@sys.driver-pi.pi/<name>.yaml` until step 3
...
## 3. Done: migrate profile/runtime paths
...
- Use the canonical `-config` name/path formatter; do not hard-code profile config directory strings
  at call sites.
...
- If both old and new exist with overlapping profile names, do not overwrite; report conflict
  clearly.
...
- `PiFs` owns canonical and legacy package path names.
- `ProfilesFs` derives profile paths from the canonical `PiFs` config dir.
- `ProfileMigrate` runs migrations in numbered order:
  - `01`: `sandbox.context.include` → `sandbox.context.append`
  - `02`: `-config/@sys.driver-pi.pi/` → `-config/@sys.driver-pi/`
- Directory migration reports only meaningful changes; internal no-op YAML skips are not exposed
  from directory-level migration.
- Menu and direct profile launches emit one clear migration message when profile configs change.
- Sandbox reports write to `.log/@sys.driver-pi/` and migrate legacy `.log/@sys.driver-pi.pi/`
  reports without clobbering conflicts.
...
- legacy sandbox report log migration and no-clobber conflicts
...
Final broad confirmation completed before bump/pub readiness review.
