...
record and every tuple are frozen independently only because native `Object.freeze` is shallow.

Repository inspection establishes the shared primitive rather than a cosmetic wrapper:

- `code/sys/std/src/m.Obj/t.ts` and `m.Obj.ts` expose no deep-freeze operation today;
- `code/sys/types/src/t/t.Readonly.ts` already owns the compile-time noun `t.DeepReadonly<T>`;
- `code/sys/fs/src/m.Pkg.Dist/u.verify/u.pinned.manifest.ts` contains the only located handwritten
  recursive implementation, `freezeJsonTree`, including cycle and deep-stack handling;
- the Driver Pi reset projection repeats nested `Object.freeze` over owned result arrays;
...
consumers adopt it in this arc.
...
Implement `deepFreeze` in `code/sys/std/src/m.Obj/u.deepFreeze.ts` without recursion.
...
- `code/sys/std/src/m.Obj/u.deepFreeze.ts`;
...
- `code/sys/std/src/m.Obj/u.deepFreeze.ts` and `m.Obj/t.ts`;
- `code/sys/std/src/m.Obj/-test/-u.deepFreeze.test.ts`;
