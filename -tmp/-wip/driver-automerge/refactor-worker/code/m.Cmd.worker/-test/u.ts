...
import { CrdtWorkerCmd } from '../m.Cmd.worker.ts';

type O = Record<string, unknown>;

/**
 * Common test utilities for Cmd-based worker CRDT repo.
 */
  ...
  const api = {
    ...
    clientRepo() {
      ...
      return {
        port1,
        port2,
        repo: CrdtWorkerCmd.Client.repo(port1),
      } as const;
    },
    ...
    /**
     * Minimal worker-like scope: routes postMessage → CrdtWorkerCmd.listen.
     */
    fakeWorkerLikeScope(repo: t.CrdtRepo) {
      ...
      CrdtWorkerCmd.Host.listen(fakeSelf, repo);
    ...
    async sample<D extends O = O>(initial: D) {
      ...
      const proxyRepo = await CrdtWorkerCmd.Client.repo(port1).whenReady();
